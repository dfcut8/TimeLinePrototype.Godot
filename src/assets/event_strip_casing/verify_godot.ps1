...
$reviewDir = Join-Path ([IO.Path]::GetTempPath()) ('strip-review-' + [guid]::NewGuid())
$previousAppData = $env:APPDATA
$assetRoot = Split-Path $PSScriptRoot
try {
    $env:APPDATA = $reviewDir
    foreach ($name in @('timeline_rail_housing','rail_recessed_light_insert','event_socket','event_connector_arm','event_strip_rear_pivot','event_strip_casing')) {
...
'@
    ...
    $run = Start-Process -FilePath $Godot -ArgumentList '--path',$reviewDir,'--script','res://assets/event_strip_casing/verify_godot.gd','--log-file',(Join-Path $reviewDir 'runtime.log') -WindowStyle Hidden -PassThru
    if (-not $run.WaitForExit(60000)) {
        Stop-Process -Id $run.Id
        throw "Validation timed out; logs at $reviewDir"
    }
    foreach ($name in @('event_strip_rear_pivot','event_strip_casing')) {
