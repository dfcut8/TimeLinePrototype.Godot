...
Godot MCP `get_state` failed because another client owns the bridge. The CLI run is the explicit fallback. Live-project MCP verification, `godot_validate_meshes` and Forward+/D3D12 review remain incomplete; imported winding/normal checks do not claim to be the MCP validator. Physics and animation are inapplicable to these static decorative caps.
