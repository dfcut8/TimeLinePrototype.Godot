...
$reviewDir = Join-Path ([IO.Path]::GetTempPath()) ('rail67-' + [guid]::NewGuid())
$previousAppData = $env:APPDATA
try {
    $env:APPDATA = $reviewDir
    foreach ($name in 'timeline_rail_housing', 'rail_recessed_light_insert', 'rail_module_joiner') {
...
'@
    ...
    $run = Start-Process -FilePath $Godot -ArgumentList '--path', $reviewDir, '--script', 'res://assets/rail_module_joiner/verify_godot.gd', '--log-file', (Join-Path $reviewDir 'runtime.log') -WindowStyle Hidden -PassThru
    if (-not $run.WaitForExit(60000)) {
        Stop-Process -Id $run.Id
        throw "Validation timed out; logs at $reviewDir"
    }
    foreach ($name in 'rail_recessed_light_insert', 'rail_module_joiner') {
